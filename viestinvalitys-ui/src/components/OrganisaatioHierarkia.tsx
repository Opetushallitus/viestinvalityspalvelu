...
export const OrganisaatioTree = (org: Organisaatio, lng: LanguageCode) => {
  ...
  return (
    <CustomTreeItem itemId={org.oid} key={org.oid} label={translateOrgName(org, lng)}>
      {Array.isArray(org.children) ? org.children.map((node) => OrganisaatioTree(node, lng)) : null}
    </CustomTreeItem>
  );
};
