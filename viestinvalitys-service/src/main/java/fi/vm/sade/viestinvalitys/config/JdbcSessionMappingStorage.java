...
public class JdbcSessionMappingStorage implements OphSessionMappingStorage {
    ...
    public HttpSession removeSessionByMappingId(String mappingId) {
        String sql = "SELECT raportointi_session_id FROM raportointi_cas_client_session WHERE mapped_ticket_id = ?";
    ...
    public void removeBySessionById(String sessionId) {
        String sql = "DELETE FROM raportointi_cas_client_session WHERE raportointi_session_id = ?";
        jdbcTemplate.update(sql, sessionId);
    }

    @Override
    public void addSessionById(String mappingId, HttpSession session) {
        String sql = "INSERT INTO raportointi_cas_client_session (mapped_ticket_id, raportointi_session_id)"
                + " VALUES (?, ?) ON CONFLICT (mapped_ticket_id) DO NOTHING";
        jdbcTemplate.update(sql, mappingId, session.getId());
    }

    @Override
    public void clean() {
        String sql = "DELETE FROM raportointi_cas_client_session"
